...
export const weightGoalAdjustments: Record<string, number> = {
  maintain: 0,
  'lose 0.5 lb/wk': -250,
  'lose 1 lb/wk': -500,
  'gain 0.5 lb/wk': 250,
...
export function calculateDailyCalories(
  ...
  goal: keyof typeof weightGoalAdjustments = 'maintain',
): { maintenance: number; goalCalories: number } {
  const maintenance = bmr * activityFactor;
  const adjustment = weightGoalAdjustments[goal] ?? 0;
